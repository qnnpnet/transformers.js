...
import { Callable, isString, dispatchCallback, pop, product, get_bounding_box } from "./utils/core.js";
...
export class ZeroShotClassificationPipeline extends Pipeline {
  ...
  constructor(options) {
    ...
    this.label2id = Object.fromEntries(
      Object.entries(this.model.config.label2id).map(([k, v]) => [k.toLowerCase(), v])
    );
...
export class FeatureExtractionPipeline extends Pipeline {
   ...
   */
  async _call(texts, { pooling = "none", normalize = false } = {}) {
    let [inputs, outputs] = await super._call(texts);
...
export class ZeroShotAudioClassificationPipeline extends Pipeline {
   ...
   * @returns {Promise<any>}
   */
  async _call(audios, candidate_labels, { hypothesis_template = "This is a sound of {}." } = {}) {
    ...
    for (let aud of audio) {
      ...
      for (let chunk of chunks) {
        ...
        if (return_timestamps === "word") {
          ...
          chunk.token_timestamps = data.token_timestamps.tolist()[0].map((x) => round(x, 2));
        } else {
          chunk.tokens = data[0];
        }
